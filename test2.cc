...
int main(){
  ...
  std::cout << "C eigen values " << C.diagonal().adjoint() << std::endl ;
  std::cout << std::endl;
  ...
  C=eigenvectors*C*eigenvectors.adjoint();

  simultaneousDiagonalization sd{A, B, C};
  ...
  std::cout << "C eigen values "<< sd.eigenvalues(2).adjoint()<< std::endl;
